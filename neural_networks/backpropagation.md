...
#### 网络记号 (notation)：M 层前馈网络

- 第 $0$ 层为输入层，第 $1,\dots,M-1$ 层为隐藏层，第 $M$ 层为输出层； $D^{(m)}$ 为第 $m$ 层的单元数。
- $W^{(m)}_{i,j}$ ：从第 $m-1$ 层单元 $j$ 连到第 $m$ 层单元 $i$ 的权重，整层权重矩阵 $\mathbf{W}^{(m)}\in\mathbb{R}^{D^{(m)}\times D^{(m-1)}}$ 。
- $b^{(m)}_i$ ：第 $m$ 层单元 $i$ 的偏置， $\mathbf{b}^{(m)}\in\mathbb{R}^{D^{(m)}}$ 。
- $z^{(m)}_i$ ：预激活 (pre-activation)，即加权和加偏置， $\mathbf{z}^{(m)}\in\mathbb{R}^{D^{(m)}}$ 。
- $a^{(m)}_i=\sigma^{(m)}(z^{(m)}_i)$ ：激活 (activation)， $\sigma^{(m)}$ 为第 $m$ 层的激活函数（逐元素作用）， $\mathbf{a}^{(m)}\in\mathbb{R}^{D^{(m)}}$ ； $\mathbf{a}^{(0)}$ 即输入 $\mathbf{x}$ 。
- $C$ ：单个样本的总代价 (total cost)； $\mathbf{t}\in\mathbb{R}^{D^{(M)}}$ ：目标向量； $L(a^{(M)}_k,t_k)$ ：第 $k$ 个输出单元的逐单元损失 (per-unit loss)。
- 下标约定：在每一层中 $i$ 标记**当前层**的单元， $j$ 标记**下一层（更靠近输入的那层）**的单元。
